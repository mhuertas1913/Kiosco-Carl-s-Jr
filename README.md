...
**El carrito también se recompone.** El panel pasa de 1520 a 922 px, pero la cabecera, las líneas del pedido, las sugerencias y el pie conservaban su tamaño del tótem: entre las tres piezas de alrededor se comían 646 px y a los artículos les quedaban 110, o sea media hamburguesa. Ahora todo eso baja de escala y el sitio se lo queda la lista, que pasa de 110 a 330 px — de medio artículo a dos enteros. Las sugerencias son lo primero que cede porque son un extra, no el pedido; los botones de cantidad se quedan en 46 px, que es el objetivo táctil que hay que acertar con el dedo.

Las sugerencias van además **en dos columnas en vez de apiladas** (`.cu-items` en fila dentro del bloque de kiosco): `renderCartUpsell()` muestra como mucho dos (`slice(0, 2)`), así que de lado a lado ocupan la mitad de alto sin encoger nada. Se reparten con `flex: 1 1 0`, de modo que si solo hay una sugerencia se queda la fila entera en vez de dejar un hueco. Entre eso y el recorte de lo de alrededor, en modo silla de ruedas caben **tres artículos completos** (la lista pasa de 110 a 465 px) y el upselling baja del 40 % del panel al 14 %. El upselling se recortó también en modo normal (de 366 a 291 px) para que los dos modos sean el mismo diseño a distinta escala y no dos cosas distintas.
